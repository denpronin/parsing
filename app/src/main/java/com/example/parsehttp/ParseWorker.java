            ...
            public void run() {
                ...
                doc = Jsoup.connect(response).userAgent("Mozila")
                    ...
                    for (Element newsItem : newsItems) {
                        parsedText.append("Title: " + newsItem.select("a").attr("title")).append("\n");
                        parsedText.append("Href: " + newsItem.select("a").attr("href")).append("\n");
                        parsedText.append("Image: " + newsItem.select("img").attr("src")).append("\n");
                        parsedText.append("Date: " + newsItem.select("p[class=category__date]").text()).append("\n");
                        parsedText.append("Notes: " + newsItem.select("p").get(1).text()).append("\n");
                        parsedText.append("\n").append("\n");
                    }
